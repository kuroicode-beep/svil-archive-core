...
export class SqliteArchiveReader {
  ...
  constructor(ctx: WorkspaceContext) {
    ...
    const row = this.db
      .prepare(
        `SELECT id FROM workspaces WHERE lower(replace(root_path, '\\', '/')) = lower(?) LIMIT 1`,
      )
      .get(normalizePathKey(ctx.root)) as WorkspaceRow | undefined;
    if (!row) {
      const fallback = this.db.prepare('SELECT id FROM workspaces ORDER BY last_opened_at DESC LIMIT 1').get() as
        | WorkspaceRow
        | undefined;
      if (!fallback) {
  ...
  countDocuments(): number {
    const row = this.db
      .prepare(
        `SELECT COUNT(*) AS count FROM documents WHERE workspace_id = ? AND is_deleted = 0`,
      )
  ...
  listDocuments(args: {
    ...
    const rows = this.db
      .prepare(
        `SELECT id, title, relative_path, category, project, updated_at
         FROM documents
  ...
  searchDocuments(args: {
    ...
    const rows = this.db
      .prepare(
        `SELECT d.id, d.title, d.relative_path, d.category, d.project, d.updated_at,
  ...
  getDocument(id: string, includeFullBody = false): DocumentDetail | null {
    if (includeFullBody) {
      throw new Error('FULL_BODY_NOT_ALLOWED');
    }
    const row = this.db
      .prepare(
        `SELECT id, title, relative_path, category, project, author, tags, updated_at
         FROM documents
         WHERE workspace_id = ? AND id = ? AND is_deleted = 0
         LIMIT 1`,
      )
      .get(this.workspaceId, id) as
      | {
          id: string;
          title: string;
          relative_path: string;
