...
export function resolveWorkspaceContext(): WorkspaceContext {
  ...
  if (!root) {
    throw new SacArchiveError(
      ...
      'Set SAC_WORKSPACE_ROOT environment variable or sidecar.config.json workspaceRoot',
    );
  }
  ...
  if (!fs.existsSync(normalized) || !fs.statSync(normalized).isDirectory()) {
    throw new SacArchiveError(
      ...
      `Workspace directory does not exist: ${maskArtifactPath(normalized)}`,
    );
  }
